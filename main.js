...
// IMPORTANT: scope these to the project modal (prevents conflicts)
...
const certClose = certificateModal?.querySelector('.modal-close');
const certOverlay = certificateModal?.querySelector('.modal-overlay');

// Open when clicking the certificate ICON (not the whole card)
document.querySelectorAll('.cert-card .cert-icon').forEach(icon => {
  icon.addEventListener('click', (e) => {
    e.stopPropagation();

    const card = icon.closest('.cert-card');
...
function openCertificateModal(pdfPath, title) {
  ...
  if (certificateDownload) {
    certificateDownload.href = pdfPath;
  }
...
function closeCertificateModal() {
  ...
  // Stop PDF render / audio / memory
  if (certificateFrame) certificateFrame.src = '';
}

// Close handlers
certClose?.addEventListener('click', closeCertificateModal);
certOverlay?.addEventListener('click', closeCertificateModal);

// ESC closes certificate modal too
document.addEventListener('keydown', (e) => {
...
downloadResumeBtn?.addEventListener('click', (e) => {
  ...
  const fileUrl = 'assets/certs/Deepak_Malviya.pdf'; // Update with the correct path to your resume PDF
